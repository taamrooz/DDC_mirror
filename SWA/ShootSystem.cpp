...
void ShootSystem::createBullet(int xV, int yV, int x, int y) {
	...
	if (ticks - shoot->last_shot >= shoot->fire_rate) {
		...
		auto cComponent = std::make_unique<CollisionComponent>(shoot->bullet_size * 2, shoot->bullet_size * 2, BulletCollisionHandler, HandlerNames::BulletCollisionHandler, entity);
