...
void ComponentFactory::AddEnemyComponents(int id, Engine::EntityManager<Component>* em, bool level_boss) {
	...
	//animations.emplace(std::make_pair<State, std::unique_ptr<Animation>>(State::DEFAULT, std::unique_ptr<Animation>(Engine::load_animation("Animations/orc_shaman_idle.png", 4)) ));
	//animations.emplace(std::make_pair<State, std::unique_ptr<Animation>>(State::RUN, std::unique_ptr<Animation>(Engine::load_animation("Animations/orc_shaman_run.png", 4) )));
	//animations.emplace(std::make_pair<State, std::unique_ptr<Animation>>(State::HIT, std::unique_ptr<Animation>(Engine::load_animation("Animations/orc_shaman_idle.png", 4) )));
	animations.at(State::DEFAULT)->scale = 3;
	animations.at(State::RUN)->scale = 3;
	animations.at(State::HIT)->scale = 3;
	auto ani = std::make_unique<AnimationComponent>(animations);
