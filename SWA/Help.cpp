...
bool Help::init() {
	...
	instructions_first_line = std::make_unique<Texture>(*Engine::load_text("manaspc.ttf", 24, { 255, 196, 0, 255 }, "Walk through the rooms, kill the enemies, pick up the elixirs or mysterious"));
	instructions_second_line = std::make_unique<Texture>(*Engine::load_text("manaspc.ttf", 24, { 255, 196, 0, 255 }, " objects from the treasure chests and head to the last rooms of the levels"));
	instructions_third_line = std::make_unique<Texture>(*Engine::load_text("manaspc.ttf", 24, { 255, 196, 0, 255 }, "                       to kill the level bosses!"));
